...
// Cache padding levels for different use cases
// ECP-A3: YAGNI - Only define what's currently needed
const (
	// MinimalPadding: ~1200 tokens - Basic AI assistant context
	// Suitable for simple Q&A scenarios
	MinimalCachePadding = `# AI Assistant Basic Context

## Core Capabilities
This AI assistant provides help across multiple domains including:
- Programming: Python, JavaScript, Go, Java, C++
- Data Analysis: SQL, pandas, statistical methods
- System Design: Architecture patterns, best practices
- Problem Solving: Debugging, optimization, code review
...
`
	...
	// StandardPadding: ~2500 tokens - Comprehensive development assistant (CURRENT DEFAULT)
	// This is the existing padding content from GetDefaultCachePadding()
	StandardCachePadding = `# Advanced AI Assistant Context
...
// PaddingLevel represents the size/complexity of cache padding
type PaddingLevel int

const (
	PaddingLevelMinimal PaddingLevel = iota
	PaddingLevelStandard
)

// GetCachePadding returns appropriate padding based on level
// ECP-B1: DRY - centralize padding selection logic
func GetCachePadding(level PaddingLevel) string {
	switch level {
	case PaddingLevelMinimal:
		return MinimalCachePadding
	case PaddingLevelStandard:
		return StandardCachePadding
	default:
		return StandardCachePadding
	}
}
